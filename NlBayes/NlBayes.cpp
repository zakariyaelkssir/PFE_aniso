...
,   const unsigned spacing
    ...
    /* last col */
	for(unsigned i = 0 ; i<p_imSize.height;i++) {
	Gradx[i*p_imSize.width+p_imSize.width]=-image[i*p_imSize.width+(p_imSize.height-2)]+image[i*p_imSize.width+(p_imSize.height-1)];
	}
	//
	centeredMat.resize((p_imSize.width)*p_imSize.height);
	offsetMat.resize((p_imSize.width)*p_imSize.height);
	resultCenteredMat.resize((p_imSize.width)*p_imSize.height);
	for(unsigned i = 0 ; i<p_imSize.height;i++) 
	for(unsigned j = 0 ; j<p_imSize.width;j++) {
		centeredMat[j*(p_imSize.width)+i]= image[j*(p_imSize.width)+i];
		offsetMat[j*p_imSize.width+i]=image[j*(p_imSize.width)+i+2];
		resultCenteredMat[j*p_imSize.width+i]=(offsetMat[j*p_imSize.width+i]-centeredMat[j*p_imSize.width+i])/(((float)spacing)*2.0);
	}

	for(unsigned i = 1 ; i<p_imSize.whc;i++) {
		Gradx[i]=resultCenteredMat[i];
	}

	for(unsigned i = 0; i<p_imSize.height;i++) {
	//	cout<<centeredMat[i*(p_imSize.width)]<<endl;
	}
...
//apply the mask to the center body of the image
  ...
  //Symetrise
	...
	//
    for(unsigned i = band; i < ny-band; i++)
    {
	for(unsigned j = band; j < nx-band; j++)
	{
	    double sum = 0;
		...
		{
		    unsigned p = (i + l -band) * nx + j + m -band;
		    sum += input[p] * mask[l * tailemask + m];
		}
	    }
	    unsigned k = i * nx + j;
	    output[k] = sum;
...
,	const float Step
	...
	px[p_imSize.width]=255;
	py[p_imSize.height]=255;
	// % Stencil Weights
	vector<float> c,c2;
	c.resize(input.size());
	...
	for(unsigned i =0;i<p_imSize.width;i++){
		for(unsigned j =0;j<p_imSize.width;j++){
		wbR1[i*p_imSize.width+j]=0.25*(abs(Dxy[nx[i]*p_imSize.height+py[j]])-Dxy[nx[i]*p_imSize.height+py[j]]+c[i*p_imSize.width+j]);
		wbL3[i*p_imSize.width+j]=0.25*(abs(Dxy[px[i]*p_imSize.height+py[j]])+Dxy[px[i]*p_imSize.height+py[j]]+c2[i*p_imSize.width+j]);
		wtR7[i*p_imSize.width+j]=0.25*(abs(Dxy[nx[i]*p_imSize.height+py[j]])+Dxy[nx[i]*p_imSize.height+py[j]]+c2[i*p_imSize.width+j]);
		wtL9[i*p_imSize.width+j]=0.25*(abs(Dxy[px[i]*p_imSize.height+py[j]])-Dxy[px[i]*p_imSize.height+py[j]]+c[i*p_imSize.width+j]);
