...
int main(int argc, char **argv)
{
	...
	//! Declarations
	vector<float> im, imNoisy, im_normalize_image, imFinal, imDiff;
...
//! Add boundaries and symetrize them
       // const unsigned h_b = imSize.height + 2 * 2;
      //  const unsigned w_b = imSize.width  + 2 * 2;
	//symetrizeImage(im,im_normalize_image,imSize,4,true);
	//symetrize(im,imDiff,w_b,h_b,1,1);
	// normalize image  
   ...
   // symetrize(im,img_sym,w_b,h_b,imSize.nChannels,2);
	...
	Filter2d(Jyy,Jyy_n,imSize,mask2,limitXSizeJ/2,limitXSizeJ);
	Filter2d(Jxy,Jxy_n,imSize,mask2,limitXSizeJ/2,limitXSizeJ);
	...
	if (saveImage("Dxx.png", Dxx, imSize, 0.f, 255.f) != EXIT_SUCCESS) {
		return EXIT_FAILURE;
	}
    if (verbose) {
        cout << "done." << endl;
